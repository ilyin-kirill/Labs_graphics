...
#define PI 3.141592653589793238f
    ...
    struct float4x4
    {
        ...
        std::vector<float> flat_vec() const
        {
            std::vector<float> res = { row[0].x, row[0].y, row[0].z, row[0].w,
    ...
    static inline float mix(const float& u, const float& v, const float& mix) { return v * mix + u * (1 - mix); }
